		...
		try {
			...
			for (AlertList alertlist : alertlists) {
				...
				if (alertlist.getService_code() != 0) {
					...
					Service_code = alertlist.getService_code();
					hbaseTable.put(keyWord, Constant.adt_cf, "sec", Service_code);
					...
					if (plcClient != null) {
						...
						logger.warn("存在service_code=" + Service_code + "对应的设备用户信息未提供，请尽快添加");
					}
				}
				hbaseTable.execute();
				hbaseTable.close();

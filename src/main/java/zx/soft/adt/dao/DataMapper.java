...
public interface DataMapper {
	...
	@Select("SELECT id,Service_code,Rule_id,Rule_name,Matching_level,Rule_action,"
			...
			+ " WHERE id >= #{from} AND id<(#{from}+" + Constant.PAGE_SIZE + ")")
	...
	@Select("SELECT id,Service_code,Net_ending_ip,Net_ending_name,Time,Net_ending_mac,Destination_ip,"
			...
			+ "Keyword24,Keyword25,vpn1_ip,vpn2_ip,vpn3_ip FROM ${tablename}" + " WHERE id >= #{from} AND id<(#{from}+"
			+ Constant.PAGE_SIZE + ")")
	...
	public GEO getGEO(@Param("tablename") String tablename, @Param("country") String country);

	@Select("SELECT COUNTRY,JD,WD FROM ${tablename} WHERE ID BETWEEN  0000000380 AND 0000000567")
	public List<GEO> getALLGEO(@Param("tablename") String tablename);
	...
	@Select("SELECT id,ipv4,begin_time,end_time,traffic FROM ${tablename} WHERE id >= #{from} AND id<(#{from}+"
			+ Constant.PAGE_SIZE + ")")
	public List<VPNTraffic> getVPNTrafficData(@Param("tablename") String tablename, @Param("from") int from);

	@Select("SELECT id,ipv4,add_time FROM ${tablename} WHERE id >= #{from} AND id<(#{from}+" + Constant.PAGE_SIZE + ")")
	...
	@Select("SELECT id,action,device,add_time,note FROM ${tablename} WHERE id>=#{from} AND id<(#{from}+"
			+ Constant.PAGE_SIZE + ")")
